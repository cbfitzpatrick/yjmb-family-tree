...
file_path = required_file("YJMB Baritone Trees Sorted.xlsx")
...
text = "The YJMB Baritone Section Family Tree"
font_size = 240
...
for node in people_dict.values():
    ...
    if node.instrument and "Baritone" not in node.instrument:
...
for root_node in root_nodes:
    ...
    for pre, fill, node in RenderTree(root_node):
        family_tree.append(f"{pre}{node.name}")
    family_trees.append(family_tree)
    print(f"\nTree starting from {root_node.name}:")
    for pre, fill, node in RenderTree(root_node):
    #     print(f"{pre}{node.name} (ID: {node.id}, Tree Number: {node.tree_number}, Has Children: {node.has_children}, End ID: {getattr(node, 'end_id', 'N/A')},  Children Nodes: {getattr(node, 'children_nodes', 'N/A')})")
        ...
        if node.instrument == "Baritone":
            if node.nickname is not None:
                display_name = format_name_with_nickname(node.name, node.nickname)
                print(f"{pre}{display_name} ({node.rat_year})")
            else:
                print(f"{pre}{node.name} ({node.rat_year})")
        elif node.instrument and "Baritone" in node.instrument:
            if node.nickname is not None:
                display_name = format_name_with_nickname(node.name, node.nickname)
                print(f"{pre}{display_name} ({node.instrument}, {node.rat_year})")
            else:
                print(f"{pre}{node.name} ({node.instrument}, {node.rat_year})")
        else:
            print(f"{pre}{node.name} ({node.instrument}, {node.rat_year})")

# Save the final image with name cards and black rectangles
base_image.save(PROJECT_DIR / f"YJMB_Baritone_Section_Family_Trees_{max_year}.png")
base_image.show()