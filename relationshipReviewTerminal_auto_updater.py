...
Manual mode never accepts a fuzzy match automatically. Automatic-updater mode creates
new rows for externally referenced people and pauses on very-high name matches. A timestamped workbook backup
...
# Automatic updater safety threshold. References below this name-similarity
# percentage are treated as externally referenced people and get a new row.
# At or above this threshold, the updater pauses for normal manual review.
AUTO_PAUSE_NAME_SCORE = 95.0
...
class WorkbookModel:
    ...
    def best_name_similarity(self, reference: RelationReference) -> tuple[float, PersonRecord | None]:
        """Return the strongest pure name-similarity percentage and matching person.

        Unlike ``fuzzy_candidates()``, this deliberately excludes RAT-year, section,
        and exact-candidate bonuses so the automatic updater's pause threshold is a
        true 0-100 percentage.
        """
        ref_name = loose_name_key(reference.name)
        if not ref_name:
            return 0.0, None
        best_score = 0.0
        best_person: PersonRecord | None = None
        for person in self.people:
            for alias in person.aliases:
                score = difflib.SequenceMatcher(None, ref_name, loose_name_key(alias)).ratio() * 100.0
                if score > best_score:
                    best_score = score
                    best_person = person
        return best_score, best_person
...
def automatic_new_draft(item: ReviewItem) -> ResolutionDraft:
    """Build the automatic updater's default new-person proposal without prompting."""
    guessed_given, guessed_family = split_name_guess(item.reference.name)
    return ResolutionDraft(
        action="new",
        target_row=None,
        given=guessed_given,
        family=guessed_family,
        year=item.reference.year,
        instrument=item.reference.instrument,
        ensure_reciprocal=True,
        replace_conflicting_vet=False,
        update_all_references=False,
    )
...
def auto_update_one(
    ...
    total: int,
    *,
    pause_score: float = AUTO_PAUSE_NAME_SCORE,
) -> str:
    """Automatically create an externally referenced person, or pause for review.

    Defaults are intentionally equivalent to choosing: New person; all prefilled
    rows are correct; reciprocate; yes/save. A very-high pure name match, or any
    validation ambiguity in the prefilled data, falls back to the normal reviewer.
    """
    name_score, best_person = model.best_name_similarity(item.reference)
    if best_person is not None and name_score >= pause_score:
        print("\n" + hr("═"))
        print(f"AUTO-UPDATER PAUSED  {index}/{total}")
        print(hr("═"))
        print(f"Reference: {item.raw}")
        print(
            f"Very high existing-name match: {name_score:.1f}% -> "
            f"{best_person.name} (row {best_person.row}, RAT {best_person.year or '?'})"
        )
        print("Manual review is required before changing this reference.")
        return review_one(model, item, index, total)

    draft = automatic_new_draft(item)
    errors = model.validate_draft(item, draft)
    if errors:
        print("\n" + hr("═"))
        print(f"AUTO-UPDATER NEEDS REVIEW  {index}/{total}")
        print(hr("═"))
        print(f"Reference: {item.raw}")
        print("The prefilled new-person row is not safe/complete enough to save automatically:")
        for error in errors:
            print(f"  - {error}")
        return review_one(model, item, index, total)

    print(
        f"\nAUTO {index}/{total}: creating {draft.canonical_relation} "
        f"from {item.source_name} {item.column_label} "
        f"(best existing-name match {name_score:.1f}%)."
    )
    try:
        target_row, messages = model.commit_resolution(item, draft)
    except PermissionError as exc:
        print(f"\nSAVE BLOCKED: {exc}")
        return "quit"
    except Exception as exc:
        print(f"\nERROR while saving automatic resolution: {exc}")
        return "quit"

    print(f"  Created/updated row {target_row}; reciprocal relationship requested.")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Automatically create rows for externally referenced people; pause for manual "
            "review on very-high name matches or incomplete/ambiguous prefilled data"
        ),
    )
    parser.add_argument(
        "--auto-pause-score",
        type=float,
        default=AUTO_PAUSE_NAME_SCORE,
        metavar="PERCENT",
        help=f"Pure name-similarity percentage that pauses automatic creation (default: {AUTO_PAUSE_NAME_SCORE:g})",
    )
...
def main() -> int:
    ...
    if not auto_mode and items:
        print("\nChoose editor mode:")
        print("  M  Manual reviewer (existing behavior)")
        print("  A  Automatic updater for exclusively externally referenced cards")
        print("  S  Scan only; print unresolved references and exit")
        print("  Q  Quit")
        mode = prompt_choice("Mode [M/a/s/q]: ", {"m", "a", "s", "q"}, default="m")
        if mode == "q":
    ...
    if auto_mode:
        if not (0.0 <= args.auto_pause_score <= 100.0):
            print("--auto-pause-score must be between 0 and 100.", file=sys.stderr)
            return 2
        print(
            "\nAutomatic updater enabled. Defaults: New person; all prefilled rows correct; "
            "reciprocate; Yes/save."
        )
        print(
            f"It pauses for manual review at >= {args.auto_pause_score:g}% pure name similarity, "
            "or when prefilled data cannot be safely validated."
        )
    else:
    ...
    while True:
        ...
        if auto_mode:
            result = auto_update_one(
                model,
                item,
                saved + skipped + 1,
                total,
                pause_score=args.auto_pause_score,
            )
        else:
